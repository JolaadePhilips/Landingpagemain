document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleSuccessfulLogin(user) {
      ...
      if (chrome && chrome.runtime && chrome.runtime.sendMessage) {
        chrome.runtime.sendMessage({ action: 'userLoggedIn', userId: user.uid }, function(response) {
        ...
        chrome.runtime.sendMessage(
          ...
          function(response) {
            if (chrome.runtime.lastError) {
              ...
              // If there's an error, try with 'localhost' instead of '127.0.0.1'
              chrome.runtime.sendMessage(
                'fcemcgjjommnhebhhckgikgglhjndnoh',
                { action: "setAuthState", isAuthenticated: true },
                function(response) {
                  if (chrome.runtime.lastError) {
                    console.error('Error sending message (localhost):', chrome.runtime.lastError);
                  } else {
                    console.log("Auth state updated in extension (localhost)", response);
                  }
                }
              );
            } else {
